...
public class SculkPatchFeature extends Feature<SculkPatchFeatureConfig> {
    ...
    private boolean canGenerate(WorldAccess world, BlockPos pos) {
        ...
        if (blockState.getBlock() instanceof SculkSpreadable) {
            ...
            return (blockState.isAir() || (blockState.isOf(Blocks.WATER) && blockState.getFluidState().isStill())) && WildDirection.stream().map(pos::offset).anyMatch(pos2 -> world.getBlockState(pos2).isFullCube(world, pos2));
        }
    }
}
