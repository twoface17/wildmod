...
public class SculkSpreadManager {
    ...
    public void tick(WorldAccess world, BlockPos pos, WildAbstractRandom random, boolean shouldConvertToBlock) {
    ...
    public static class Cursor {
        ...
        public void spread(WorldAccess world, BlockPos pos, WildAbstractRandom random, SculkSpreadManager spreadManager, boolean shouldConvertToBlock) {
            if (this.canSpread(world, pos, spreadManager.worldGen)) {
                if (this.update > 0) {
                    ...
                    if (shouldConvertToBlock && sculkSpreadable.spread(world, this.pos, blockState, this.faces, spreadManager.isWorldGen())) {
                        ...
                        world.playSound(null, this.pos, RegisterSounds.BLOCK_SCULK_SPREAD, SoundCategory.BLOCKS, 1.0F, 1.0F);
                    }
        ...
        private static List<Vec3i> shuffleOffsets(WildAbstractRandom random) {
            return net.frozenblock.wildmod.registry.Util.copyShuffled(OFFSETS, random);
        }

        @Nullable
        private static BlockPos getSpreadPos(WorldAccess world, BlockPos pos, WildAbstractRandom random) {
