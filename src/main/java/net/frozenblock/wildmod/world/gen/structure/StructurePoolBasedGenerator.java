...
public class StructurePoolBasedGenerator {
        ...
        if (structurePoolElement == EmptyPoolElement.INSTANCE) {
            ...
            if (optional.isPresent()) {
                ...
                if (optional3.isEmpty()) {
                    LOGGER.error("No starting jigsaw {} found in start pool {}", identifier, (registryEntry.getKey().get()).getValue());
                    return Optional.empty();
                }
    ...
    public static boolean method_43729(
            ServerWorld serverWorld, RegistryEntry<StructurePool> registryEntry, Identifier identifier, int i, BlockPos blockPos, boolean bl
    ) {
        ChunkGenerator chunkGenerator = serverWorld.getChunkManager().getChunkGenerator();
        StructureManager structureManager = serverWorld.getStructureManager();
        StructureAccessor structureAccessor = serverWorld.getStructureAccessor();
        AbstractRandom abstractRandom = serverWorld.getRandom();
        ...
        Optional<StructureType.StructurePosition> optional = generate(context, registryEntry, Optional.of(identifier), i, blockPos, false, Optional.empty(), 128);
        if (optional.isPresent()) {
            ...
            for(StructurePiece structurePiece : structurePiecesCollector.toList().pieces()) {
                if (structurePiece instanceof PoolStructurePiece poolStructurePiece) {
                    poolStructurePiece.generate(serverWorld, structureAccessor, chunkGenerator, (Random) abstractRandom, BlockBox.infinite(), blockPos, bl);
                }
            }
    ...
    static final class StructurePoolGenerator {
        ...
        void generatePiece(
            ...
            label139:
            for(Structure.StructureBlockInfo structureBlockInfo : structurePoolElement.getStructureBlockInfos(this.structureManager, blockPos, blockRotation, this.random)) {
                ...
                if (optional.isPresent()
                        && (((StructurePool)optional.get()).getElementCount() != 0 || Objects.equals(identifier, StructurePools.EMPTY.getValue()))) {
                    ...
                    if (optional2.isPresent()
                            && (((StructurePool)optional2.get()).getElementCount() != 0 || Objects.equals(identifier2, StructurePools.EMPTY.getValue()))) {
                                ...
                                if (modifyBoundingBox && blockBox2.getBlockCountY() <= 16) {
                                    l = list2.stream().mapToInt(structureBlockInfox -> {
                                        if (!blockBox2.contains(structureBlockInfox.pos.offset(JigsawBlock.getFacing(structureBlockInfox.state)))) {
                                            ...
                                            int ix = optionalx.map(pool -> pool.getHighestY(this.structureManager)).orElse(0);
                                            int jx = optional2x.map(pool -> pool.getHighestY(this.structureManager)).orElse(0);
                                        ...
                                        if (!VoxelShapes.matchesAnywhere(
                                            ...
                                            if (minY + 1 <= this.maxSize) {
                                                this.structurePieces
                                                        .addLast(
                                                                new StructurePoolBasedGenerator.ShapedPoolStructurePiece(poolStructurePiece, mutableObject2, minY + 1)
                                                        );
                                            }
                                            continue label139;
                                        }
