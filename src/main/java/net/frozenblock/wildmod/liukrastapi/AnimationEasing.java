...
public class AnimationEasing {
	...
	public static float easeInOutExpo(float f) {
		return f == 0 ? 0 : f == 1 ? 1 : f < 0.5 ? MathAddon.pow(2, 20 * f - 10) / 2
				: (2 - MathAddon.pow(2, -20 * f + 10)) / 2;
	}

	public static float easeInCirc(float f) {
		return 1 - MathHelper.sqrt(1F - MathAddon.pow(f, 2));
	}

	public static float easeOutCirc(float f) {
		return MathHelper.sqrt(1 - MathAddon.pow(f - 1, 2));
	}

	public static float easeInOutCirc(float f) {
		return f < 0.5 ? (1 - MathHelper.sqrt(1 - MathAddon.pow(2 * f, 2))) / 2
				: (MathHelper.sqrt(1 - MathAddon.pow(-2 * f + 2, 2)) + 1) / 2;
	}

	public static float easeInBack(float f) {
		float c1 = 1.70158F;
		float c3 = c1 + 1F;
		return c3 * MathAddon.pow(f, 3) - c1 * MathAddon.pow(f, 2);
	}

	public static float easeOutBack(float f) {
		float c1 = 1.70158F;
		float c3 = c1 + 1F;
		return 1F + c3 * MathAddon.pow(f - 1F, 3) + c1 * MathAddon.pow(f - 1, 2);
	}

	public static float easeInOutBack(float f) {
		float c1 = 1.70158F;
		float c2 = c1 + 1F;
		return f < 0.5 ? (MathAddon.pow(2 * f, 2) * ((c2 + 1) * 2 * f - c2)) / 2
				: (MathAddon.pow(2 * f - 2, 2) * ((c2 + 1) * (f * 2 - 2) + c2) + 2) / 2;
	}

	public static float easeInElastic(float f) {
		float c4 = (2F * MathHelper.PI) / 3;
		return f == 0 ? 0 : f == 1 ? 1 : -MathAddon.pow(2F, 10F * f - 10F) *
				MathHelper.sin((f * 10F - 10.75F) * c4);
	}

	public static float easeOutElastic(float f) {
		float c4 = (2F * MathHelper.PI) / 3;
		return f == 0 ? 0 : f == 1 ? 1 : MathAddon.pow(2F, -10F * f) *
	...
	public static float easeInOutElastic(float f) {
		...
		return f == 0 ? 0 : f == 1 ? 1 : f < 0.5F ?
				-(MathAddon.pow(2F, 20F * f - 10F) * MathHelper.sin((f * 10F - 11.125F) * c5)) / 2
				: (MathAddon.pow(2, -20F * f + 10) * MathHelper.sin((20F * f - 11.125F) * c5)) / 2 + 1;
	}
