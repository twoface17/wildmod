...
import frozenblock.wild.mod.fromAccurateSculk.SculkCatalystPhase;
...
public class SculkCatalystBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world2, BlockState blockState2, BlockEntityType<T> blockEntityType) {
        if (!world2.isClient) {
            return SculkCatalystBlock.checkType(blockEntityType, NewBlockEntityType.SCULK_CATALYST, (world, blockPos, blockState, sculkCatalystBlockEntity) -> sculkCatalystBlockEntity.getEventListener().tick(world));
        }
        return null;
    }
