...
interface StatusEffectFogModifier {
    ...
    public default void applyStartEndModifier(FogData fogData, LivingEntity livingEntity, frozenblock.wild.mod.liukrastapi.StatusEffectInstance statusEffectInstance, float f, float g) {
    ...
    static class BlindnessFogModifier implements StatusEffectFogModifier {
        ...
        public void applyStartEndModifier(FogData fogData, LivingEntity livingEntity, frozenblock.wild.mod.liukrastapi.StatusEffectInstance statusEffectInstance, float f, float g) {
            float h = MathHelper.lerp(Math.min(1.0F, (float)statusEffectInstance.getDuration() / 20.0F), f, 5.0F);
            if (fogData.fogType == BackgroundRenderer.FogType.FOG_SKY) {
                fogData.fogStart = 0.0F;
                fogData.fogEnd = h * 0.8F;
    ...
    static class DarknessFogModifier implements StatusEffectFogModifier {
        ...
        public float applyColorModifier(LivingEntity livingEntity, frozenblock.wild.mod.liukrastapi.StatusEffectInstance statusEffectInstance, float f, float g) {
