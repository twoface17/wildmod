...
public class SonicBoomTask extends Task<WardenEntity> {
    ...
    public SonicBoomTask() {
        super(ImmutableMap.of(MemoryModuleType.ATTACK_TARGET, MemoryModuleState.VALUE_PRESENT, RegisterEntities.SONIC_BOOM_COOLDOWN, MemoryModuleState.VALUE_ABSENT, RegisterEntities.SONIC_BOOM_SOUND_COOLDOWN, MemoryModuleState.REGISTERED, RegisterEntities.SONIC_BOOM_SOUND_DELAY, MemoryModuleState.REGISTERED), RUN_TIME);
    }
    ...
    protected void keepRunning(ServerWorld serverWorld, WardenEntity wardenEntity, long l) {
        if (!wardenEntity.getBrain().hasMemoryModule(RegisterEntities.SONIC_BOOM_SOUND_DELAY) && !wardenEntity.getBrain().hasMemoryModule(RegisterEntities.SONIC_BOOM_SOUND_COOLDOWN)) {
