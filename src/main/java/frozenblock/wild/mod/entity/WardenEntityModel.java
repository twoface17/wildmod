...
public class WardenEntityModel extends EntityModel<WardenEntity> {
    ...
    public void setAngles(WardenEntity entity, float limbSwing, float limbSwingAmount, float animationProgress, float netHeadYaw, float headPitch) {
        ...
        this.rig.pivotY = rigY + 40 + (
                AnimationAPI.easeInSine(0, 0.68f, 0, time)
                + AnimationAPI.easeInSine(0.68f, 1.04f, -10F, time)
                + AnimationAPI.easeInBack(1.04f, 1.2f, +2F, time)
                + AnimationAPI.easeInBack(1.2f, 3.24f, 0F, time)
                + AnimationAPI.easeInSine(3.24f, 3.92f, -17F, time)
                + AnimationAPI.easeInSine(3.92f, 4.6f, -5F, time)
                + AnimationAPI.easeOutSine(4.6f, 4.76f, +1.13F, time)
                + AnimationAPI.easeInSine(4.76f, 5.48f, -1.13F, time)
                + AnimationAPI.easeInSine(5.48f, 5.84f, -10F, time)
                );
