...
public class SculkSensorListener implements GameEventListener {
    ...
    private static boolean isOccluded(World world, Vec3d start, Vec3d end) {
        frozenblock.wild.mod.liukrastapi.Vec3d vec3d = new frozenblock.wild.mod.liukrastapi.Vec3d((double)MathHelper.floor(start.x) + 0.5, (double)MathHelper.floor(start.y) + 0.5, (double)MathHelper.floor(start.z) + 0.5);
        frozenblock.wild.mod.liukrastapi.Vec3d vec3d2 = new frozenblock.wild.mod.liukrastapi.Vec3d((double)MathHelper.floor(end.x) + 0.5, (double)MathHelper.floor(end.y) + 0.5, (double)MathHelper.floor(end.z) + 0.5);
        Direction[] var5 = Direction.values();
        int var6 = var5.length;

        for(int var7 = 0; var7 < var6; ++var7) {
            Direction direction = var5[var7];
            frozenblock.wild.mod.liukrastapi.Vec3d vec3d3 = (frozenblock.wild.mod.liukrastapi.Vec3d) vec3d.withBias(direction, 9.999999747378752E-6);
    ...
    public interface Callback {
        ...
        default boolean canAccept(net.minecraft.world.event.GameEvent gameEvent, frozenblock.wild.mod.event.GameEvent.Emitter arg) {
    ...
    public static record Vibration(GameEvent gameEvent, int distance, Vec3d pos, @Nullable UUID uuid, @Nullable UUID projectileOwnerUuid, @Nullable Entity entity) {
        public static final Codec<Vibration> CODEC = RecordCodecBuilder.create((instance) -> {
            return instance.group(Registry.GAME_EVENT.getCodec().fieldOf("game_event").forGetter(Vibration::gameEvent), Codecs.NONNEGATIVE_INT.fieldOf("distance").forGetter(Vibration::distance), frozenblock.wild.mod.liukrastapi.Vec3d.CODEC.fieldOf("pos").forGetter(Vibration::pos), UUID.optionalFieldOf("source").forGetter((vibration) -> {
