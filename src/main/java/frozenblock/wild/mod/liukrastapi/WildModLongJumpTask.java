...
public class WildModLongJumpTask<E extends MobEntity> extends Task<E> {
    ...
    private static final List<Integer> field_37431 = Lists.newArrayList(new Integer[]{65, 70, 75, 80});
    ...
    protected boolean shouldKeepRunning(ServerWorld serverWorld, MobEntity mobEntity, long l) {
        boolean bl = this.lastPos.isPresent() && ((Vec3d)this.lastPos.get()).equals(mobEntity.getPos()) && this.cooldown > 0 && !mobEntity.isInsideWaterOrBubbleColumn() && (this.lastTarget != null || !this.targets.isEmpty());
    ...
    protected void run(ServerWorld serverWorld, E mobEntity, long l) {
        ...
        int j = blockPos.getY();
        int k = blockPos.getZ();
        this.targets = (List)BlockPos.stream(i - this.horizontalRange, j - this.verticalRange, k - this.horizontalRange, i + this.horizontalRange, j + this.verticalRange, k + this.horizontalRange).filter((blockPos2) -> {
            return !blockPos2.equals(blockPos);
        }).map((blockPos2) -> {
            return new WildModLongJumpTask.Target(blockPos2.toImmutable(), MathHelper.ceil(blockPos.getSquaredDistance(blockPos2)));
    ...
    protected void keepRunning(ServerWorld serverWorld, E mobEntity, long l) {
        if (this.lastTarget != null) {
            if (l - this.targetTime >= 40L) {
                ...
                serverWorld.playSoundFromEntity((PlayerEntity)null, mobEntity, (SoundEvent)this.entityToSound.apply(mobEntity), SoundCategory.NEUTRAL, 1.0F, 1.0F);
            }
        } else {
            --this.cooldown;
    ...
    protected void method_41342(ServerWorld serverWorld, E mobEntity, long l) {
        while(true) {
            if (!this.targets.isEmpty()) {
                ...
                WildModLongJumpTask.Target target = (WildModLongJumpTask.Target)optional.get();
    ...
    protected Optional<Target> getTarget(ServerWorld world) {
        ...
        List<WildModLongJumpTask.Target> var10001 = this.targets;
